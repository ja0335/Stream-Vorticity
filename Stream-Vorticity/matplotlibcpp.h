...
namespace matplotlibcpp {
	...
	bool contour(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::vector<Numeric> &z, const std::string& s = "")
	{
		...
		for (size_t i = 0; i < x.size(); ++i) 
		{
			...
			PyList_SetItem(zlist, i, PyFloat_FromDouble(z.at(i)));
		}

		PyObject* plot_args = PyTuple_New(3);
		PyTuple_SetItem(plot_args, 0, xlist);
		PyTuple_SetItem(plot_args, 1, ylist);
		PyTuple_SetItem(plot_args, 2, zlist);
		PyTuple_SetItem(plot_args, 3, pystring);

		PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_contour, plot_args);
